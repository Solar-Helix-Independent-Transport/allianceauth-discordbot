...
from aadiscordbot.models import QuoteMessage
...
class Quote(commands.Cog):
    ...
    quote_commands = SlashCommandGroup(
        "quote",
        "Save and Recall Quotes (memes)",
    ...
    @commands.message_command(
        name="Save Quote",
        description="Save this message as a Quote",
        guild_ids=app_settings.get_all_servers()
    )
    ...
    async def savequote(self, ctx, message):
        ...
        try:
            ...
            return await ctx.respond("Reference is not Unique, Please try again", ephemeral=True)
        except Exception as e:
            return await ctx.respond(e)

    @quote_commands.command(
        name="recall",
        description="Recall a Quote from its reference",
        guild_ids=app_settings.get_all_servers()
    )
    ...
    @commands.user_command(
        name="View Quotes",
        description="View a users saved Quotes",
        guild_ids=app_settings.get_all_servers()
    )
